...
docstring — driven by `versa consolidate-session` and `run_interactive`'s
turn-count-gated auto-trigger on exit, never by `handle_turn` itself.
"""
...
# The post-response steps of ONE turn (write the memory fact, record the
# interaction, record diagnostics) when that turn was started with
# `defer_tail=True`: `SessionLoop._tail` appends them here instead of
# awaiting them, and `handle_turn` runs them as one background task once the
# answer has been returned. None (the default) means "run each step inline,
# in place" -- exactly the pre-deferral behavior every existing caller and
# test still gets. A ContextVar for the same reason as streaming.delta_sink:
# one shared loop serves many concurrent turns.
...
class SessionLoop:
    def __init__(
        ...
        # PER SESSION (session_id -> that session's unresolved options
        # generation): one shared loop serves many concurrent chats, so a
        # single value here would let one chat's pending options be
        # superseded (or resolved) by another's turn.
        self._prior_disambiguation_turn_ids: dict[UUID, UUID] = {}
        ...
        # The still-running deferred post-response tail of each session's
    ...
    async def run_interactive(self, learner_id: UUID) -> UUID:
        ...
        print(f"versa: new session {session_id}")
        print("versa: type your message. ctrl-D or empty line + ctrl-C to exit.")
        turn_index = 0
        ...
        if turn_index >= self._memory_config.min_turns_for_cli_auto_consolidation:
            ...
            if result is not None:
                print(
                    f"versa: consolidated this session's path — "
    ...
    async def handle_turn(
        ...
        *,
        ...
        first words long before the full answer exists. The return value is
        still the complete text. A sink that raises stops receiving pieces;
        the turn itself carries on. Only `FinalAnswer` streams -- the
        "which of these did you mean?" turn and BASELINE mode return whole.

        `defer_tail`: return as soon as the answer is ready and run the
        post-response persistence (memory fact, interaction record,
        diagnostics) as a background task instead of before returning. The
        SAME steps run in the SAME order either way; only when they run
        differs. The next turn of this session waits for the tail to finish
        before it starts, so nothing ever reads state that is half-written.
        A turn that raises never schedules its tail. Default False: every
        step runs inline, as it always has.
        """
        ...
        embed_token = None
        begin_turn = getattr(self._embedding_client, "begin_turn", None)
        if begin_turn is not None:
            embed_token = begin_turn()
        sink_token = (
    ...
    def ablation_config(self) -> AblationConfig:
        """The mode every session this loop starts runs under (fixed for the
        loop's lifetime; sessions persist it at creation)."""
        return self._ablation

    async def pending_options(self, session_id: UUID) -> list[Option]:
        """The clickable options this session's latest turn offered that the
        student has not resolved yet (empty if none) -- what a client shows
        after `handle_turn` returns the "which of these did you mean?"
        message."""
        generation_id = self._prior_disambiguation_turn_ids.get(session_id)
        if generation_id is None or self._disambiguation is None:
            return []
        return [
            o
    ...
    def _guard_sink(sink: DeltaSink) -> DeltaSink:
        """A client that disconnects mid-answer must not turn a good answer
        into a failed turn: after the first sink error, stop calling it and
        let the model finish so the answer is still persisted."""
        alive = True

        async def _guarded(piece: str) -> None:
            nonlocal alive
            if not alive:
                return
            try:
                ...
                alive = False
                logger.warning("delta sink failed (%s); finishing the turn without streaming", exc)

        return _guarded

    async def _tail(self, fn, /, *args, **kwargs):
        """Run one post-response step: inline (returning its result) by
        default, or -- when this turn was started with `defer_tail=True` --
        queue it for the background tail and return None immediately. Only
        steps whose result the turn does not use go through here."""
        steps = _TAIL_STEPS.get()
        if steps is None:
            return await fn(*args, **kwargs)
        steps.append((fn, args, kwargs))
        return None

    def _tail_duration_ms(self, start: float) -> float | None:
        """`turn_diagnostics.duration_ms` for a deferred turn is frozen at the
        moment the step is queued (~ when the answer was ready), so it keeps
        meaning "how long the user waited" rather than absorbing the
        background tail. None (compute at write time, as before) inline."""
        return (time.monotonic() - start) * 1000 if _TAIL_STEPS.get() is not None else None

    def _schedule_session_tail(self, session_id: UUID, steps: list) -> None:
        # Run the tail in a copy of this turn's context (so it keeps the
        # turn's embedding cache) but with streaming and deferral switched off.
        ctx = contextvars.copy_context()
        ctx.run(_TAIL_STEPS.set, None)
        ctx.run(delta_sink.set, None)
        ...
        async def _run() -> None:
            for fn, args, kwargs in steps:
                try:
                    ...
                    logger.warning(
                        "deferred post-response step %s failed for session %s: %s",
                        getattr(fn, "__name__", fn), session_id, exc, exc_info=True,
                    )

        task = asyncio.get_running_loop().create_task(_run(), context=ctx)
        self._background_tasks.add(task)
        task.add_done_callback(self._background_tasks.discard)
        self._session_tails[session_id] = task

        def _clear(t: asyncio.Task, sid: UUID = session_id) -> None:
            if self._session_tails.get(sid) is t:
                self._session_tails.pop(sid, None)

        task.add_done_callback(_clear)

    async def _await_session_tail(self, session_id: UUID) -> None:
        task = self._session_tails.get(session_id)
        if task is not None:
    ...
    async def _handle_disambiguation_turn(
        ...
        if selected_option_id is not None:
            ...
            if option is not None:
                ...
                if self._interactions_enabled:
                    ...
                    await self._tail(self._interaction_options.mark_selected, option.id)
                await self._tail(
        ...
        # context instead.
        typed_past_note = ""
        prior_turn_id = self._prior_disambiguation_turn_ids.pop(session_id, None)
        ...
        # either. The option texts are read by the caller (the CLI or any other client)
        ...
        if self._interactions_enabled:
            await self._tail(_options_tail)
    ...
    async def consolidate_session(self, session_id: UUID):
        ...
        `versa consolidate-session` command and `run_interactive`'s own
        turn-count-gated auto-trigger on exit — never SessionLoop itself.
    ...
    async def _record_disambiguation_diagnostics(
        ...
        await self._diagnostics.record(
            TurnDiagnostics(
                ...
                entropy_bits=None,
                duration_ms=(
                    duration_ms if duration_ms is not None else (time.monotonic() - start) * 1000
                ),
                warnings=warnings,
                teach_failed=teach_failed,
                retry_count=_total_retry_count(self._tiers) - retry_count_start,
