...
from versa.domain_config import Domain
...
class NodeCall(BaseModel):
    """One row from node_calls — for read paths that need
...
class DisambiguationTurnKind(str, Enum):
    """`disambiguation_turns.kind` (migration 058) — which mechanism
    produced this turn's branches/options, since the two resolve via
    completely different logic once a click comes in (see loop.py's
    `_handle_disambiguation_turn`).

    AMBIGUITY: the ordinary flow — AssessAndBranch judged the message
    genuinely ambiguous, DisambiguationOptions phrased LLM-generated
    readings. A click means "this reading is what I meant."
    REASON_CONFIRMATION: a REASON-based memory match (migration 056,
    `matched_via="reason"`) offered directly to the student as a fixed
    yes/no, instead of the LLM-judged `ConfirmFactMatch` a situation/
    resolution match still uses — see IDEAS.md's "ask for confirmation
    directly" entry. A click means "yes/no, that reason applies,"
    never a reading choice. No AssessAndBranch or DisambiguationOptions
    call happens for this kind — the two branches/options are fixed
    Python literals (`disambiguate.REASON_CONFIRM_YES_TEXT`/
    `REASON_CONFIRM_NO_TEXT`), not LLM output.
    """

    AMBIGUITY = "ambiguity"
    REASON_CONFIRMATION = "reason_confirmation"
    # Dormant: only legacy rows (removed Guess Mode, migrations 061/062) carry this.
...
class DisambiguationTurn(BaseModel):
    ...
    `kind` (migration 058) defaults to AMBIGUITY, so every pre-existing
    row and every row the ordinary AssessAndBranch/DisambiguationOptions
    path writes reads back exactly as before this field existed — see
    `DisambiguationTurnKind`'s own docstring for the other value.
    """
...
class TurnDiagnostics(BaseModel):
    ...
    so a read-side client can read it directly instead of
    re-deriving it.
    ...
    # Which of EmbedAndSearchFacts' two independent searches (migration
    # 056/057) produced matched_fact_id -- None when there was no match
    # (or memory is disabled). A "reason" match is routed to the direct
    # confirmation flow below instead of ConfirmFactMatch (see
    # reason_confirmation_offered); a "situation_resolution" match is
    # purely observational here, changing nothing about whether
    # branching was skipped.
    memory_match_via: str | None = None
    # migration 058/059/060, IDEAS.md "ask for confirmation directly":
    # a REASON match was shown to the student as a direct yes/no
    # instead of going through ConfirmFactMatch. reason_confirmed_by_
    # student is None until answered (same "NULL covers both no-match
    # and disabled" shape as matched_fact_id), then True/False on the
    # turn the student actually clicked.
...
class HistoryTurn(BaseModel):
    ...
    # The sandbox-chat claim-update flow's inline note ("Noted: ...",
    # reviews.apply_stated_preference_to_claims), replayed from
    # student_reviews (migration 068) on resume -- None on every ordinary
...
class LearnerFactType(str, Enum):
    ...
    # REASON-based memory match (clicked "Yes, that's right" — see
    # IDEAS.md's "ask for confirmation directly" entry and
    # DisambiguationTurnKind.REASON_CONFIRMATION). Durably, queryably
    # stronger evidence than DIRECT_ANSWER: that one may also follow a
    # memory match, but only an LLM's (ConfirmFactMatch) say-so, never
    # the student's own.
...
class LearnerFact(BaseModel):
    ...
    `reason`/`reason_embedding` (migration 056) are additive and
    NULLABLE, unlike every field above — a reason is written only when
    this specific exchange actually suggested one (see
    memory.WriteLearnerFact and IDEAS.md's "Store the reason" entry),
    never invented to fill the column, and every fact written before
    this pair of columns existed reads back with both None. Retrieval
    over `reason_embedding` is a second, independently-searched path
    alongside `embedding`, not a replacement for it — see
    memory.EmbedAndSearchFacts for how the two are compared.
...
class ExtractedFact(BaseModel):
    ...
    `reason` is None whenever the model had nothing to add beyond the
    resolution itself — a valid, expected output (see
    memory._fact_prompt), not a parse failure the way a missing
    situation/resolution would be."""
...
class FactSearchResult(BaseModel):
    ...
    `matched_via` names which of the two independent searches
    (situation+resolution text, or reason text — see
    memory.EmbedAndSearchFacts) produced this candidate. For a
    "reason" match it is also a real behavioral signal, not just
    measurement: `_handle_disambiguation_turn` routes it to the direct
    yes/no confirmation flow instead of `ConfirmFactMatch` entirely
    (IDEAS.md "ask for confirmation directly") — for "situation_
    resolution" it changes nothing, ConfirmFactMatch's judgment call
    is the same gate as before.

    `reason` is the matched fact's own `reason` text (None if it has
    none) — carried here so the confirmation flow above can phrase the
    question ("it looks like this is because {reason}") without a
    second lookup."""
    ...
    matched_via: Literal["situation_resolution", "reason"] | None = None
...
class ReasonRelevance(BaseModel):
    """ConfirmReasonRelevance's output — a REASON-embedding hit (unlike
    a situation/resolution one) is matched on abstract, short text
    ("you prefer X before Y"), which embedding similarity cannot
    reliably tell apart from its own negation: "stop giving me X
    before Y" is topically almost identical to the reason it rejects.
    A live run confirmed this concretely -- an explicit reversal of a
    stated preference still cleared the similarity bar and got offered
    back as if it were being reconfirmed. `still_applies=False` is how
    that gets caught before the student ever sees the confirmation
    question — see memory.ConfirmReasonRelevance's own docstring."""
...
# memory layer: that name is already taken (prior verification transcripts
# use "story" to mean a `LearnerFact` row). See migration 034's own header comment.
