...
(versa.llm.LLMClient), but for vectors instead of text.
...
# The in-flight/finished embeddings of ONE turn, keyed by (text, task_type).
# A ContextVar (like streaming.delta_sink) so the cache follows the turn into
# the tasks it spawns (the deferred post-response tail inherits it) without
# being threaded through every signature, and one shared client can serve
# many concurrent turns without their caches touching.
_TURN_CACHE: ContextVar[dict[tuple[str, str | None], asyncio.Future] | None] = ContextVar(
    "versa_turn_embedding_cache", default=None
)


class TurnCachedEmbeddings:
    """EmbeddingClient wrapper that de-duplicates identical embeds WITHIN a
    turn. Measured on the real pipeline: one turn embedded the student's
    same message 3 times (memory search, history block, interaction
    recorder) at ~0.6-0.8s each. Inside `begin_turn()`..`end_turn()` an
    identical (text, task_type) shares one API call -- including when two
    callers ask concurrently. Outside a turn (consolidation, scripts) it is
    a plain pass-through, so nothing changes for callers that never opt in.

    A failed call is evicted, not cached: the next identical request
    retries instead of replaying the failure."""

    def __init__(self, inner: EmbeddingClient) -> None:
        self._inner = inner

    def begin_turn(self) -> Token:
        return _TURN_CACHE.set({})
    ...
    async def embed(self, text: str, *, task_type: str | None = None) -> list[float]:
        ...
        task = cache.get(key)
        if task is None:
            task = asyncio.ensure_future(self._inner.embed(text, task_type=task_type))
            cache[key] = task

            def _evict_if_failed(t: asyncio.Future, key=key, cache=cache) -> None:
                if t.cancelled() or t.exception() is not None:
                    cache.pop(key, None)

            task.add_done_callback(_evict_if_failed)
