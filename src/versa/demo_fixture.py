...
(`statement_embedding` is a zero vector — nothing here depends on
...
WHAT THIS DOES NOT BUILD: the comparison VIEW that reads these two
portraits and checks whether one predicts the other's picks
(comparison.py, `versa compare-portraits`) — this module is only the
reproducible DATA that comparison needs, not the comparison itself.
"""
...
from versa.models import Claim, ClaimSource, ClaimStatus, ClaimWritePolicy, Learner, StatedPreferenceLabel
