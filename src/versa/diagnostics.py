...
duration, warnings, whether Teach itself failed) so a read-side
client can read it directly instead of re-deriving anything — see the
"zero business logic in the UI" constraint this store exists to
satisfy.
...
from versa.ablation import AblationConfig, AblationCostSummary
