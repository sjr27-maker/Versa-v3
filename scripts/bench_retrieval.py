...
Usage:
    uv run python scripts/bench_retrieval.py --seed        # build the 100k rows (once)
    uv run python scripts/bench_retrieval.py --bench        # run the two requirements
    uv run python scripts/bench_retrieval.py --explain       # print EXPLAIN ANALYZE
    uv run python scripts/bench_retrieval.py --all           # all three, in order
"""
...
from versa.models import EntryState
from versa.retrieval import RetrievalContext, stage1_filter, stage2_recall
...
def _database_url() -> str:
    ...
    url = os.getenv("DATABASE_URL") or os.getenv(
        "VERSA_TEST_DATABASE_URL", "postgresql://versa:versa@localhost:5434/versa"
    )
    return url
